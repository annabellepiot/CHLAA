...
public:
  ...
  static void update(real_type time, real_type dt, const real_type* state, const shared_state& shared, internal_state& internal, rng_state_type& rng_state, real_type* state_next) {
    ...
    const real_type p_EI = 1 - monty::math::exp(-dt / shared.incubation_time);
    const real_type p_rec_asym = 1 - monty::math::exp(-dt / shared.duration_asym);
    const real_type p_stage = 1 - monty::math::exp(-dt / shared.time_to_next_stage);
    const real_type p_leave_sym = 1 - monty::math::exp(-dt / shared.duration_sym);
    const real_type p_wane_Ra = 1 - monty::math::exp(-dt / shared.immunity_asym);
    const real_type p_wane_Rs = 1 - monty::math::exp(-dt / shared.immunity_sym);
    const real_type p_wane_V1 = 1 - monty::math::exp(-dt / shared.vax_immunity_1);
    const real_type p_wane_V2 = 1 - monty::math::exp(-dt / shared.vax_immunity_2);
    ...
    const real_type orc_cap_step = monty::math::floor(orc_active * shared.orc_capacity * dt);
    const real_type ctc_cap_step = monty::math::floor(ctc_active * shared.ctc_capacity * dt);
    ...
    const real_type vax1_cap_step = monty::math::floor(vax1_active * shared.vax1_doses_per_day * dt);
    const real_type vax2_cap_step = monty::math::floor(vax2_active * shared.vax2_doses_per_day * dt);
    ...
    const real_type p_inf = 1 - monty::math::exp(-lambda * dt);
